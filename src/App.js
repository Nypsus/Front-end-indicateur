...
[
  ...
  const fetchPriceFromIFPS = async (productId) => {
    try {
      // Récupère les données depuis l'IPFS via Pinata
      const response = await fetch(`https://gateway.pinata.cloud/ipfs/Qme4BKfGHuMukLnbf7LqZTqK6snH57eFbENeqVzLfg28eg/${productId}`);
      ...
      // Vérifie que les données sont valides et contiennent un prix valide
      if (data && typeof data.price === 'number' && data.price > 0) {
        return data.price;  // Renvoie le prix du produit si valide
      } else {
        throw new Error("Prix invalide ou manquant dans les données.");
      }
  ...
  const sendTransaction = async (amountIn) => {
    ...
    try {
      ...
      const gasLimit = 5000;  // Limite de gaz pour la transaction
  ...
  const handlePayment = async () => {
    ...
    let amountIn = 0;
    try {
      // Vérification du prix du produit via IFPS ou autre source
      const fetchedPrice = await fetchPriceFromIFPS(productId); // Fonction asynchrone pour récupérer le prix du produit
      if (fetchedPrice <= 0) {
        alert("Le prix du produit est invalide ou n'a pas pu être récupéré.");
        setLoading(false);
        return;  // Si le prix est invalide, ne pas continuer avec la transaction
      }
  
      // Utilisation du prix récupéré
      amountIn = fetchedPrice;
  
      // Vérification de la devise et calcul du montant à envoyer
      if (selectedCurrency === 'USD') {
        // Pas de conversion nécessaire si en USD
        amountIn = fetchedPrice; // Le prix est déjà en USD
        ...
        amountIn = fetchedPrice / bnbToUsdRate;
      } else {
