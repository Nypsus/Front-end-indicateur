...
function App() {
  ...
  const getBNBToUSDTRate = async () => {
    try {
      ...
      setBnbToUsdRate(bnbToUsdRate);
  
      // Calculer le taux USDT -> BNB (en supposant que 1 USDT = 1 USD)
      const usdtToBnbRate = 1 / bnbToUsdRate;  // Calculer le taux USDT -> BNB
      setUsdtToBnbRate(usdtToBnbRate);
  
      console.log(`Taux de conversion USDT -> BNB : 1 USDT = ${usdtToBnbRate} BNB`);
  ...
  const handlePayment = async () => {
    ...
    if (selectedProductId) {
      try {
        ...
        // Récupérer le prix en USDT du produit
        const productPriceInUsdt = product.price;
        console.log(`Prix du produit : ${productPriceInUsdt} USDT`);
        ...
        // Calculer le montant à envoyer en BNB en fonction du taux de conversion
        let productPriceInBnb = productPriceInUsdt * usdtToBnbRate;
  
        // Limiter à 18 décimales pour éviter les erreurs de fraction
        productPriceInBnb = ethers.utils.formatUnits(ethers.utils.parseUnits(productPriceInBnb.toFixed(18), 18), 18);
  
        console.log(`Prix du produit en BNB : ${productPriceInBnb}`);
        ...
        const estimatedGas = await contract.estimateGas.pay(
          ethers.constants.AddressZero,
          productPriceInUsdt, // Envoi de l'USDT comme paramètre
          selectedProductId,
          { value: amountInWei }
        );
        ...
        console.log('Frais de gas estimés en BNB :', ethers.utils.formatUnits(totalGasCost, 'ether'));
  
        const userBalance = await provider.getBalance(address);
        console.log('Solde de l\'utilisateur en BNB :', ethers.utils.formatUnits(userBalance, 'ether'));
  
        if (userBalance.lt(totalGasCost)) {
          alert("Vous n'avez pas assez de BNB pour couvrir les frais de gas.");
          return;
        }
        ...
        console.log("Transaction prête à être envoyée : ", tx);
