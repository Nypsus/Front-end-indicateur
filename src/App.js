...
[
  ...
  const sendTransaction = async (amountIn) => {
    ...
    try {
      ...
      // Convertir le montant en fonction de la devise
      if (selectedCurrency === 'USD') {
        amountInWei = ethers.utils.parseUnits(amountIn.toString(), 18); // Convertir le montant USD en wei
      } else if (selectedCurrency === 'BNB') {
        // Si c'est BNB, on convertit le montant en BNB directement
        amountInWei = ethers.utils.parseUnits(amountIn.toString(), 18);
      }
  
      // Récupérer le prix du gaz actuel
      const gasPrice = await provider.getGasPrice();
  
      // Définir la limite de gaz
      const gasLimit = 200000;  // Une valeur fixe, tu peux l'ajuster selon tes besoins
  
      // Estimer les frais de gaz de manière plus prudente (ajouter 10% au prix actuel)
      const adjustedGasPrice = gasPrice.add(gasPrice.div(10));
  
      // Vérifier le solde et les frais de gaz
      const walletBalance = await provider.getBalance(signer.getAddress());
  
      // Calculer le coût total (montant de la transaction + frais de gaz)
      const totalCost = amountInWei.add(adjustedGasPrice.mul(gasLimit));
      ...
      const tx = {
        to: contractAddress,
        value: amountInWei,  // Le montant en wei
        gasLimit: gasLimit,
        gasPrice: adjustedGasPrice, // Utilisation des frais ajustés
      };
